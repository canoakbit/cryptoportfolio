...
# Generate random returns for other strategies
np.random.seed(42)  # For reproducibility
random_multiplier = 1.25

# Random performance generation for each strategy
for strategy in ['CNN-L', 'DNN-L', 'LSTM-L', 'DQ-L', 'PPO-L', 'DQ_CL-L', 'PPO_CL-L',
    ...
    merged_data[strategy] = merged_data['Avg_Crypto'].apply(
        lambda x: np.random.uniform(-random_multiplier * abs(x), random_multiplier * abs(x))
    )
